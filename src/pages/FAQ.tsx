...
export function FAQ() {
  // FAQPage structured data is built from the nine CONFIRMED answers only.
  // Questions still awaiting client confirmation are excluded — see
  // src/data/schema.ts.
  usePageMeta(
    'FAQ & Policies | Mobile Massage in Brooklyn | The Living Axis',
    'Travel, deposits, preparation and policies for mobile massage therapy across Brooklyn and greater New York City.',
    faqPageSchema
  );
  ...
  const childrenPolicy =
    confirmedFaqs.find((f) => f.question === 'Do you work with children or teenagers?')?.answer ?? '';
    ...
    <div className="bg-brand-ivory min-h-screen pt-32 pb-0">
          ...
          {confirmedFaqs.map((faq, index) => (
            <div key={index} className="border-b border-brand-teal/20">
              <button
                ...
                className="w-full py-6 md:py-8 flex justify-between items-center text-left focus:bg-brand-teal/5 transition-colors px-4 -mx-4 rounded-md group"
      ...
      <section className="bg-brand-teal/5 py-24 md:py-32 px-6 lg:px-12 border-t border-brand-teal/10">
        <div className="max-w-[1000px] mx-auto">
          ...
          >
            <h2 className="font-display text-[40px] md:text-[56px] text-brand-teal mb-6">Policies & Standards</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
            ...
            >
              ...
              {/* Deposit disclosure, surface 2 of 3 (Services page, here, booking step). */}
              <p className="text-brand-espresso/80 leading-relaxed italic mb-2">{DEPOSIT_REASON}</p>
              <p className="text-brand-espresso/80 leading-relaxed">{DEPOSIT_TERMS}</p>
            </motion.div>
            ...
            >
              ...
              <p className="text-brand-espresso/80 leading-relaxed">
                Standard travel and mobile setup are fully included in all listed rates within the normal service area.
                Extended travel outside the normal service area may require a custom quote.
              </p>
            </motion.div>
            ...
            >
              ...
              <p className="text-brand-espresso/80 leading-relaxed mb-8">
                The Living Axis provides professional therapeutic massage and wellness services only. All sessions are
                conducted in strict accordance with professional therapeutic standards and boundaries.
              </p>

              {/*
                CHILDREN & ADOLESCENTS.
                This policy lives ONLY here and in the FAQ answer above. It must
                never appear on the homepage, in an audience tile, in a service
                card, or in any image. No imagery of minors appears anywhere on
                the site.
              */}
              <h3 className="font-display text-2xl text-brand-teal mb-4">Children &amp; Adolescents</h3>
              <p className="text-brand-espresso/80 leading-relaxed">{childrenPolicy}</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-brand-teal text-brand-ivory py-24 md:py-32 px-6 lg:px-12 text-center">
        ...
        >
          <h2 className="font-display text-[40px] md:text-[56px] leading-tight mb-8">Still have a question?</h2>
