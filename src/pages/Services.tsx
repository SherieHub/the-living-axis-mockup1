...
export function Services() {
  usePageMeta(
    'Massage Services & Pricing in Brooklyn | The Living Axis',
    ...
    <div className="bg-brand-ivory min-h-screen pt-32 pb-0">
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16">
                <div className="md:col-span-7 lg:col-span-8">
                  <p className="text-brand-espresso/80 text-lg leading-relaxed mb-8">{service.description}</p>
              ...
              {index === 0 && (
                <div className="mt-24 w-full max-w-4xl mx-auto">
                  <SiteImage slot="services-neck-shoulder" />
                </div>
              )}
              {index === 1 && (
                <div className="mt-24 w-full max-w-4xl mx-auto">
                  <SiteImage slot="services-prenatal" />
                </div>
              )}
              {index === 2 && (
                <div className="mt-24 w-full max-w-4xl mx-auto">
                  <SiteImage slot="services-sports" />
                </div>
              )}
            </motion.div>
          ))}
      ...
      <section className="bg-brand-teal/5 py-24 md:py-32 px-6 lg:px-12 border-t border-brand-teal/10">
        <div className="max-w-[1200px] mx-auto">
          ...
          >
            <h2 className="font-display text-[36px] md:text-[48px] text-brand-teal mb-4">
            ...
            {specialtyServices.map((specialty, i) => (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-6 gap-4">
                  ...
                  {/* Standalone durations first, then the add-on price if the
                      service is also offered as an add-on. Reflexology, Scalp
                      Therapy and Body Scrub are all bookable on their own. */}
                ...
                <p className="text-brand-espresso/80 leading-relaxed mb-10 flex-grow">{specialty.description}</p>
                ...
                <div className="mt-auto">
                  {specialty.standalone ? (
                    ...
                    <p className="text-center text-xs font-semibold uppercase tracking-widest text-brand-espresso/50 py-3">
                      Added during booking
                    </p>
                  )}
                </div>
              </motion.div>
          ...
          <div className="mt-20 w-full max-w-4xl mx-auto">
            <SiteImage slot="services-reflexology" />
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 lg:px-12 max-w-3xl mx-auto text-center border-t border-brand-teal/10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={fadeUp}>
          ...
          <div className="space-y-4 text-brand-espresso/70 leading-relaxed">
            <p>
              <strong>Travel &amp; Setup:</strong> Standard travel and full professional setup are included for all
              primary Brooklyn locations. Extended travel to other boroughs, Long Island, or New Jersey may require a
              custom quote.
            </p>
            {/* Deposit disclosure, surface 1 of 3 (here, FAQ, booking step). */}
            <div>
              <p className="italic mb-2">{DEPOSIT_REASON}</p>
              <p>
                <strong>Booking Deposit:</strong> {DEPOSIT_TERMS}
              </p>
            </div>
          </div>
        </motion.div>
      ...
      <section className="ground-dark bg-brand-teal text-brand-ivory py-24 md:py-32 px-6 lg:px-12 text-center">
        ...
        >
          <h2 className="font-display text-[40px] md:text-[56px] leading-tight mb-10">Ready for your session?</h2>
          <Link
            to="/book"
